#!/bin/bash
# ============================================================================
# NVMe Cloud Qualification Suite (Enterprise Edition)
# ============================================================================
# Features:
# 1. Multi-drive synchronized parallel testing with dynamic Excel scaling.
# 2. Smart NUMA node detection and thread binding (via fio or numactl).
# 3. Targeted workload sampling for extreme time-saving in multi-drive mode.
# 4. Strict error handling, dependency checking, and auto-retry mechanisms.
# 5. Modular test switches and customizable block sizes.
# ============================================================================
...
# 1. Execution Mode
# - single : Full Cartesian matrix traversal (Generates detailed matrix sheets).
# - multi  : Targeted sampling for multi-drive (Generates dynamic summary sheet).
TEST_MODE="multi"

# 2. Target Devices (Array)
# Space-separated list of block devices. E.g., ("/dev/nvme0n1" "/dev/nvme1n1")
# [DANGER] All data on these drives will be irreversibly erased!
...
MIX_RUNTIME=1800      # Mixed RW test duration in seconds (Standard: 1800)
QOS_RUNTIME=3600      # QoS test duration in seconds (Standard: 3600)

DO_SEQ_PRECON="yes"   # Precondition before Seq tests (128k Write)
SEQ_PRE_LOOPS=2       # Number of full drive passes for Seq precondition
DO_RAND_PRECON="yes"  # Precondition before Rand tests (4k Rand Write)
RAND_PRE_LOOPS=1      # Number of full drive passes for Rand precondition

# 5. Test Phase Switches (yes / no)
RUN_SEQ_READ="yes"
RUN_SEQ_WRITE="yes"
RUN_RAND_READ="yes"
RUN_RAND_WRITE="yes"
...
# 6. Block Size Whitelist (Space separated)
# Remove any block size you do not wish to test.
TEST_BS_LIST="4k 8k 16k 32k 64k 128k 256k 512k 1m"

# ============================================================================
# [ Smart NUMA Binding Configuration ]
# ============================================================================

ENABLE_NUMA_BIND="yes"       # Enable NUMA detection and binding
NUMA_BIND_METHOD="fio"       # "fio" (uses --numa_cpu_nodes) or "numactl" (wrapper)
NUMA_FALLBACK_NODE="0"       # Fallback node if sysfs detection fails
...
# Targeted Combos for Multi-drive dynamic table
SEQ_COMBOS =[
...
def generate_excel(df_all, mixed_results, args, devs, drive_infos):
    ...
    with pd.ExcelWriter(out_excel, engine='openpyxl') as writer:
        ...
        # 1. Detailed Matrix Sheets (Only generated in single mode)
        if args.mode == 'single':
            ...
            # Only consider the first drive for detailed sheets in single mode
        ...
        # 3. Dynamic Summary Sheet (Multi-Drive)
