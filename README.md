...
这是最核心的工具，分中英文版。
全自动跑完顺序、随机、混合读写的性能矩阵（涵盖各种 BS 和 QD）。
跑完后会调用 Python 引擎自动生成 Excel 报表，直接看 IOPS、带宽和时延。
...
验证 nvme format 到底有没有把数据清干净。
原理：执行完格式化后，脚本会对全盘进行 Hex Dump 扫描，如果发现任何非 0 数据就会报错。

槽位上/下电 (nvme_slot_power_test.sh)
不需要手动拔盘，直接通过 PCI 槽位的 power 接口切断和恢复供电。
用于测试 SSD 在反复掉电重启过程中的初始化稳定性。

驱动加载压力 (nvme_driver_visible_check.sh)
循环执行 modprobe 卸载和加载 nvme 驱动，检查内核日志里是否有识别错误。

PPU / AI 监控与压测脚本
硬件状态实时采集 (collect_ppu_monitor.sh)
依赖 ppudbg 命令。
同时对多个设备（默认 ID 0 和 1）进行后台监控。
会自动把 Stress (压力)、Power (功耗)、ICN (内部互联) 和 Video (视频处理) 的实时信息分别记录到不同的日志文件中。

自动化压测循环 (trace_player_autorun.sh)
依赖 trace_player 执行程序。
按照设定的总时间（比如 30000 秒）循环跑指定的测试流（multi_stream.txt）。
模拟长时间的高负载业务流，检查设备在持续运行下的表现。
